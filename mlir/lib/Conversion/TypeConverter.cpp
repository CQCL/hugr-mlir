...
struct HugrTypeConverter : OneToNTypeConverter  {
    ...
    FailureOr<SmallVector<Value>> materializeTargetSum(OpBuilder&, TypeRange, Value, Location) const;
    FailureOr<SmallVector<Value>> materializeTargetTuple(OpBuilder&, TypeRange, Value, Location) const;
    FailureOr<Value> materializeSourceSum(OpBuilder&, hugr_mlir::SumType, ValueRange, Location) const;
    FailureOr<Value> materializeSourceTuple(OpBuilder&, TupleType, ValueRange, Location) const;
};

}


// void hugr_mlir::populateHugrToLLVMConversionPatterns(mlir::RewritePatternSet& ps, mlir::TypeConverter const& tc, int benefit) {

//     ps.add<ConvertHugrFuncToFuncConversionPattern>(tc, ps.getContext(), benefit);
// }
mlir::FailureOr<mlir::OneToNTypeMapping> HugrTypeConverter::partitionSumType(hugr_mlir::SumType st) const {
...
mlir::FailureOr<mlir::OneToNTypeMapping> HugrTypeConverter::partitionTupleType(TupleType tt) const {
...
mlir::FailureOr<mlir::SmallVector<mlir::Value>> HugrTypeConverter::materializeTargetSum(OpBuilder& rw, TypeRange ts, Value src, Location loc) const {
    auto st = llvm::dyn_cast<hugr_mlir::SumType>(src.getType());
    if(!st) { return failure(); }

    auto mb_partition = partitionSumType(st);
    if(failed(mb_partition)) { return failure(); }

    SmallVector<Type> expected_tys{rw.getIndexType()};
    llvm::copy(mb_partition->getConvertedTypes(), std::back_inserter(expected_tys));
    if(expected_tys != ts) { return failure(); }
    ...
    for(auto i = 0; i < mb_partition->getOriginalTypes().size(); ++i) {
        ...
        llvm::transform(mb_partition->getConvertedTypes(i), std::back_inserter(vec), [&rw,loc](auto t) {
            return rw.createOrFold<ub::PoisonOp>(loc, t, rw.getAttr<ub::PoisonAttr>());
        });
    };

    SmallVector<int64_t> cases;
    for(auto i = 1; i < st.numAlts(); ++i) {cases.push_back(i);}

    auto switch_op = rw.create<scf::IndexSwitchOp>(loc, ts, tag, cases, cases.size());
    for(auto r: switch_op.getRegions()) {
        ...
        for(auto i = 0; i < mb_partition->getOriginalTypes().size(); ++i) {
            if(i != n) {
                ...
                auto variant_v = rw.createOrFold<hugr_mlir::ReadVariantOp>(loc, src_t, src, n);
                auto mb_converted_variant_v = this->materializeTargetConversion(rw, loc, mb_partition->getConvertedTypes(i), variant_v);
...
mlir::FailureOr<mlir::SmallVector<mlir::Value>> HugrTypeConverter::materializeTargetTuple(OpBuilder& rw, TypeRange ts, Value src, Location loc) const {
    auto tt = llvm::dyn_cast<TupleType>(src.getType());
    if(!tt) { return failure(); }
...
mlir::FailureOr<mlir::Value> HugrTypeConverter::materializeSourceSum(OpBuilder& rw, hugr_mlir::SumType st, ValueRange vs, Location loc) const {
    if(st.numAlts() == 0) { return failure(); }
    ...
    SmallVector<Type> expected_tys{rw.getIndexType()};
    llvm::copy(mb_partition->getConvertedTypes(), std::back_inserter(expected_tys));
    if(expected_tys != vs.getTypes()) { return failure(); }
    ...
    SmallVector<int64_t> cases;
    for(auto i = 1; i < st.numAlts(); ++i) {cases.push_back(i);}
    auto switch_op = rw.create<scf::IndexSwitchOp>(loc, st, tag, cases, cases.size());
    OpBuilder::InsertionGuard _g(rw);

    for(auto [r, src_t]: llvm::zip_equal(switch_op.getRegions(), st.getTypes())) {
        assert(r->empty() && "Region should not have been constructed with block");
        auto n = r->getRegionNumber();
        rw.setInsertionPointToStart(rw.createBlock(r));
        ...
        if(!this->isLegal(src_t)) {
            v = materializeSourceConversion(rw, loc, src_t, mb_partition->getConvertedValues(converted_vs, n));
        } else {
            assert(mb_partition->getConvertedValues(converted_vs, n).getTypes() == TypeRange{src_t} && "must");
...
mlir::FailureOr<mlir::Value> HugrTypeConverter::materializeSourceTuple(OpBuilder& rw, TupleType tt, ValueRange vs, Location loc) const {
    auto mb_partition = partitionTupleType(tt);
    if(failed(mb_partition) || mb_partition->getConvertedTypes() != vs.getTypes()) { return failure(); }
...
HugrTypeConverter::HugrTypeConverter() : OneToNTypeConverter() {
    addConversion([this](TupleType tt, llvm::SmallVectorImpl<Type>& result) -> LogicalResult {
        auto p = partitionTupleType(tt);
        if(succeeded(p)) {
            llvm::copy(p->getConvertedTypes(), std::back_inserter(result));
            return success();
        }
        return failure();
    });
    addConversion([this](hugr_mlir::SumType st, llvm::SmallVectorImpl<Type>& result) -> LogicalResult {
        auto p = partitionSumType(st);
        if(succeeded(p)) {
    ...
    addConversion([this](hugr_mlir::FunctionType t) -> std::optional<Type> {
        SmallVector<Type> inputs;
        if(failed(this->convertTypes(t.getArgumentTypes(), inputs))) {
            return std::nullopt;
        }
        SmallVector<Type> outputs;
        if(failed(this->convertTypes(t.getResultTypes(), outputs))) {
            return std::nullopt;
        }
    ...
    addSourceMaterialization([this](OpBuilder& rw, TupleType tt, ValueRange vs, Location loc) -> std::optional<Value> {
        auto p = this->materializeSourceTuple(rw, tt, vs, loc);
        if(failed(p)) { return std::nullopt; }
        return *p;
    });
    addTargetMaterialization([this](OpBuilder& rw, TypeRange ts, Value src, Location loc) -> std::optional<SmallVector<Value>> {
        auto tt = llvm::dyn_cast<TupleType>(src.getType());
    ...
    addArgumentMaterialization([this](OpBuilder& rw, TupleType tt, ValueRange vs, Location loc) -> std::optional<Value> {
        auto p = this->materializeSourceTuple(rw, tt, vs, loc);
        if(failed(p)) { return std::nullopt; }
        return *p;
    });

    addSourceMaterialization([this](OpBuilder& rw, hugr_mlir::SumType st, ValueRange vs, Location loc) -> std::optional<Value> {
    ...
    addArgumentMaterialization([this](OpBuilder& rw, hugr_mlir::SumType st, ValueRange vs, Location loc) -> std::optional<Value> {
        auto p = this->materializeSourceSum(rw, st, vs, loc);
        if(failed(p)) { return std::nullopt; }
        return *p;
    });
    ...
    //     .getResult(0);
    // });
    // addTargetMaterialization([&](OpBuilder &builder, TypeRange resultType, Value inputs,
    //                            Location loc) -> std::optional<Value> {
    //   if (inputs.size() != 1)
    //     return std::nullopt;
