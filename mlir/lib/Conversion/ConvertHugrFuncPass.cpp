...
static hugr_mlir::ModuleOp getOwningModule(mlir::Operation *op) {
    for(;;) {
        ...
        if(auto r = llvm::dyn_cast<hugr_mlir::ModuleOp>(op)) { return r; }
        if(op->hasTrait<mlir::OpTrait::IsIsolatedFromAbove>()) { return nullptr; }
        op = op->getParentOp();
    }
}
...
struct PreConvertHugrFuncPass : hugr_mlir::impl::PreConvertHugrFuncPassBase<PreConvertHugrFuncPass> {
  using  PreConvertHugrFuncPassBase::PreConvertHugrFuncPassBase;
  using FuncClosureMap_t = llvm::DenseMap<StringAttr, TypedValue<hugr_mlir::FunctionType>>;
  // LogicalResult initialize(MLIRContext*) override;
  void runOnOperation() override;
};

struct ConvertHugrFuncPass : hugr_mlir::impl::ConvertHugrFuncPassBase<ConvertHugrFuncPass> {
...
struct ClosureiseCallOp : OpConversionPattern<hugr_mlir::CallOp> {
    ...
    ClosureiseCallOp(PreConvertHugrFuncPass::FuncClosureMap_t& fcm, Args&& ...args) : OpConversionPattern(std::forward<Args>(args)...), func_closure_map(fcm) {}
...
struct ClosureiseLoadConstantOp : OpConversionPattern<hugr_mlir::LoadConstantOp> {
    template<typename ...Args>
    ClosureiseLoadConstantOp(PreConvertHugrFuncPass::FuncClosureMap_t& fcm, Args&& ...args) : OpConversionPattern(std::forward<Args>(args)...), func_closure_map(fcm) {}
...
struct ClosureiseConstantOp : OpConversionPattern<hugr_mlir::ConstantOp> {
    template<typename ...Args>
    ClosureiseConstantOp(PreConvertHugrFuncPass::FuncClosureMap_t& fcm, Args&& ...args) : OpConversionPattern(std::forward<Args>(args)...), func_closure_map(fcm) {}
...
struct LowerSwitch : OpConversionPattern<hugr_mlir::SwitchOp> {
  ...
  LogicalResult matchAndRewrite(
      hugr_mlir::SwitchOp, OpAdaptor, ConversionPatternRewriter&) const override;
};

struct HugrFuncToFunc : OpConversionPattern<hugr_mlir::FuncOp> {
    template<typename ...Args>
    HugrFuncToFunc(PreConvertHugrFuncPass::FuncClosureMap_t& fcm, Args&& ...args) : OpConversionPattern(std::forward<Args>(args)...), func_closure_map(fcm) {}
...
struct OutputToReturn : OpConversionPattern<hugr_mlir::OutputOp> {
  using OpConversionPattern::OpConversionPattern;
  LogicalResult matchAndRewrite(
      hugr_mlir::OutputOp, OpAdaptor, ConversionPatternRewriter&) const override;
};

struct LowerAllocFunction : OpConversionPattern<hugr_mlir::AllocFunctionOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(hugr_mlir::AllocFunctionOp, OpAdaptor, ConversionPatternRewriter&) const override;
};

struct CallIndirectOpSignatureConversion : public OpConversionPattern<func::CallIndirectOp> {
  using OpConversionPattern::OpConversionPattern;

  /// Hook for derived classes to implement combined matching and rewriting.
  LogicalResult
  matchAndRewrite(func::CallIndirectOp callOp, OpAdaptor adaptor,
...
LogicalResult OutputToReturn::matchAndRewrite(
    hugr_mlir::OutputOp op, OpAdaptor adaptor, ConversionPatternRewriter& rw) const {
...
LogicalResult ClosureiseConstantOp::matchAndRewrite(hugr_mlir::ConstantOp op, OpAdaptor adaptor, ConversionPatternRewriter& rw) const {
    auto a = llvm::dyn_cast<hugr_mlir::StaticEdgeAttr>(op.getValue());
    if(!a || llvm::isa<hugr_mlir::FunctionType>(a.getType())) { return failure(); }
...
LogicalResult ClosureiseLoadConstantOp::matchAndRewrite(hugr_mlir::LoadConstantOp op, OpAdaptor adaptor, ConversionPatternRewriter& rw) const {
...
LogicalResult ClosureiseCallOp::matchAndRewrite(hugr_mlir::CallOp op, OpAdaptor adaptor, ConversionPatternRewriter& rw) const {
...
LogicalResult HugrFuncToFunc::matchAndRewrite(hugr_mlir::FuncOp op, OpAdaptor adaptor, ConversionPatternRewriter &rw) const {
    ...
    // TODO we are throwing away arg attrs for closure args. probs fine, but we could put them in this dics
        SmallVector<Attribute> new_arg_attrs{rw.getDictionaryAttr({})};
        llvm::copy(arg_attrs.getValue().drop_front(op.getCaptures().size()), std::back_inserter(new_arg_attrs));
        list.append(
    ...
    auto converted_tuple_type = llvm::cast<TupleType>(getTypeConverter()->convertType(op.getFunctionType()));
    assert(converted_tuple_type.size() == 2 && "by conversion");
    auto converted_func_type = llvm::cast<FunctionType>(converted_tuple_type.getType(0));

    rw.setInsertionPointToEnd(&module.getBody().front());
    auto func = rw.create<func::FuncOp>(
        loc, op.getSymName(), converted_func_type,
        list.getAttrs());
    ...
    if(!op.getBody().empty()) {
        ...
        for(auto ba: old_entry->getArguments().drop_front(op.getCaptures().size())) {
            new_arg_locs.push_back(ba.getLoc());
        }
        assert(new_arg_locs.size() == converted_func_type.getNumInputs() && "wrong num of locs");

        rw.updateRootInPlace(func, [&] {
            auto entry_block = rw.createBlock(&func.getBody(), func.getBody().begin(), converted_func_type.getInputs(), new_arg_locs);
            rw.setInsertionPointToStart(entry_block);
            SmallVector<Value> replacements;
            rw.createOrFold<hugr_mlir::ReadClosureOp>(replacements, loc, adaptor.getCaptures().getTypes(), entry_block->getArgument(0));
            llvm::copy(entry_block->getArguments().drop_front(), std::back_inserter(replacements));
...
LogicalResult LowerAllocFunction::matchAndRewrite(hugr_mlir::AllocFunctionOp op, OpAdaptor adaptor, ConversionPatternRewriter &rw) const {
    assert(getTypeConverter() && "must have type converter");
    auto loc = op.getLoc();
    rw.setInsertionPoint(op);
    SmallVector<Value> replacements;
    auto res_t = llvm::cast<TupleType>(getTypeConverter()->convertType(op.getOutput().getType()));
    assert(res_t.size() == 2 && "conversion ensures it");
    auto func = rw.createOrFold<func::ConstantOp>(op.getLoc(), res_t.getType(0), op.getFunc().getRef().getLeafReference());
...
LogicalResult HugrCallToCall::matchAndRewrite(hugr_mlir::CallOp op, OpAdaptor adaptor, ConversionPatternRewriter &rw) const {
    auto v = adaptor.getCalleeValue();
    if(!v) { return failure(); }

    auto tt = llvm::dyn_cast<TupleType>(v.getType());
    assert(tt && tt.size() == 2 && llvm::isa<FunctionType>(tt.getType(0)) && llvm::isa<hugr_mlir::ClosureType>(tt.getType(1)) && "conversion not working");
...
void PreConvertHugrFuncPass::runOnOperation() {
    ...
    getOperation()->walk<WalkOrder::PostOrder>([&hugr_funcs](hugr_mlir::FuncOp op) {
        hugr_funcs.push_back(op);
    });
...

    {
        ...
        ps.add<ClosureiseCallOp,ClosureiseConstantOp,ClosureiseLoadConstantOp>(func_closures, context);
    ...
    for(auto f: hugr_funcs) {
        ...
        for(auto v: f.getCaptures()) { captures.insert(v); }
        ...
        rw.updateRootInPlace(f, [&] {
            ...
            llvm::transform(captures, std::back_inserter(new_launch_locs), [](auto x) { return x.getLoc(); });
            for(auto ba: old_launch_block->getArguments().drop_front(orig_num_caps)) {
                new_launch_tys.push_back(ba.getType());
                new_launch_locs.push_back(ba.getLoc());
            }

            auto new_launch_block = rw.createBlock(old_launch_block, new_launch_tys, new_launch_locs);
            ...
            llvm::copy(new_launch_block->getArguments().drop_front(captures.size()), std::back_inserter(replacements));
    ...
    if(failed(applyPartialConversion(getOperation(), target, {}))) {
        emitError(getOperation()->getLoc()) << "Failed to legalize funcs to have no non-local edges";
        return signalPassFailure();
    }
...
void ConvertHugrFuncPass::runOnOperation() {
    ...
    getOperation()->walk<WalkOrder::PostOrder>([&fcm](hugr_mlir::AllocFunctionOp op) {
    ...
    type_converter->addConversion([tc=type_converter.get()](hugr_mlir::FunctionType t) -> std::optional<Type> {
        ...
        return rw.getType<TupleType>(SmallVector<Type>{rw.getFunctionType(new_arg_tys,t.getResultTypes()), new_arg_tys[0]});
    });
    ...
    target.addDynamicallyLegalOp<func::FuncOp>([tc=type_converter.get()](func::FuncOp op){
        return tc->isSignatureLegal(op.getFunctionType());
    });
    target.addDynamicallyLegalOp<cf::BranchOp,cf::SwitchOp,cf::CondBranchOp>([tc=type_converter.get()](Operation* op){
        return isLegalForBranchOpInterfaceTypeConversionPattern(op, *tc);
    });
    target.addDynamicallyLegalOp<func::ReturnOp>([tc=type_converter.get()](func::ReturnOp op){
        return isLegalForReturnOpTypeConversionPattern(op, *tc);
    });
    target.addDynamicallyLegalOp<func::CallOp>([tc=type_converter.get()](func::CallOp op){
        return tc->isSignatureLegal(op.getCalleeType());
    });
    target.addDynamicallyLegalOp<func::CallIndirectOp>([tc=type_converter.get()](func::CallIndirectOp op){
