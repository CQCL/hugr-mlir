...
mlir::ChangeResult AllPredecessors::join(std::optional<DenseSet<Block*>> const& rhs) {
  if(!blocks) { return ChangeResult::NoChange; }
  if(!rhs) { blocks = rhs; return ChangeResult::Change; }
...
mlir::LogicalResult AllPredecessorsAnalysis::visit(ProgramPoint pp) {
  ...
  if(predecessors->allPredecessorsKnown()) {
    for(auto op: predecessors->getKnownPredecessors()) {
      cr != all_preds->join(getOrCreateFor<AllPredecessors>(pp, op->getBlock()));
    }
  } else {
...
hugr_mlir::VerifyLinearityAnalysis::VerifyLinearityAnalysis(mlir::Operation* top_, mlir::AnalysisManager&) : top(top_) {}

hugr_mlir::VerifyLinearityAnalysis::BlockSet_t const& hugr_mlir::VerifyLinearityAnalysis::getPreds(mlir::Operation* op) {
...
void HugrVerifyLinearityPass::runOnOperation() {
  ...
  getOperation()->walk([&](mlir::Operation* op) {
    for(auto const& o: op->getResults()) { try_value(o); }
    for(auto &r: op->getRegions()) {
      for(auto &b: r.getBlocks()) {
        for(auto a: b.getArguments()) { try_value(a); }
      }
    }
  });
