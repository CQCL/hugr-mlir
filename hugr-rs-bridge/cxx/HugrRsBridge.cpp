...
auto hugr_rs_bridge::parse_hugr_json(mlir::Location loc, llvm::StringRef str)
    -> mlir::FailureOr<hugr_unique_ptr<Hugr>> {
...
auto hugr_rs_bridge::parse_hugr_rmp(
    mlir::Location loc, llvm::ArrayRef<uint8_t> bytes)
    -> mlir::FailureOr<hugr_unique_ptr<Hugr>> {
...
auto hugr_rs_bridge::hugr_to_json(mlir::Location loc, Hugr const& hugr)
...
auto hugr_rs_bridge::hugr_to_rmp(mlir::Location loc, Hugr const& hugr)
    -> mlir::FailureOr<std::vector<uint8_t>> {
