...
#
{ llvmPackages_git, lib, python3Packages, runCommandNoCC, llvm-src, symlinkJoin
...
let
  ...
  python3 =
    let packages = ps: [ ps.numpy ps.pybind11 ps.psutil ps.pyyaml ps.lit ];
    in python3Packages.python.withPackages packages // {
      overridePackages = f:
        python3Packages.python.withPackages (ps: f ps (packages ps));
    };
    ...
    let
      mlir = (self.llvm.override (old: {
        ...
        passthru = old.passthru or { } // {
          python = python3;
          mlir-src = src;
          all-outputs = symlinkJoin {
            name = "${src.name}-all-outputs";
            paths = builtins.map (x: mlir.${x}) mlir.outputs;
          };
        };
        ...
        in old.cmakeFlags or [ ] ++ [
          ...
          "-DLLVM_BUILD_LLVM_SHLIB=OFF"
          "-DLLVM_LINK_LLVM_SHLIB=OFF"
        ];
      });
    in { inherit mlir; });
in llvmTools.mlir
